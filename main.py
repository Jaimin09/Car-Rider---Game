...
explosion_sound = pygame.mixer.Sound('Explosion+3.wav')
Finish_Applause = pygame.mixer.Sound('applause.wav')
pygame.mixer.music.load('gameStart.wav')
...
homepage = pygame.image.load('lamborgini.png')
...
mainroad = pygame.image.load('mainRoad.png')
mainroad2 = pygame.image.load('mainRoad2.png')
playerCar1 = pygame.image.load('car1.png')
    ...
    if but_position[0]+but_position[2]> cur[0] > but_position[0] and but_position[1]+but_position[3] > cur[1] > but_position[1]:
        ...
        if click[0] == 1 and action != None :
            if action == "play":
                ...
                pygame.mixer.music.load('carhorn.wav')
                pygame.mixer.music.play(-1)
                gameLoop()
            if action == 'level2':
                gameLoop2()
...
def gameLoop2():
    ...
    block_list = pygame.sprite.Group()
    player1 = player.Player(player1_carX,player1_carY, 'car1')
    player2 = player.Player(player2_carX, player2_carY, 'car2')
    for i in range(int((backgroundImageRepeat-1)*1.8)):
        block = obstacle.Obstacle()
        block.rect.x = random.randrange(roadStart,roadEnd,51)
        block.rect.y = random.randrange(-((backgroundImageRepeat-1)*700)+700,0,100)
    ...
    while not gameExit :
        ...
        player1_carY += player1_car_changeY
        player1_carX += player1_car_changeX
        player2_carX += player2_car_changeX

        if player1_carY <= 500:
            player1_carY = 500
        if player1_carY >= 570:
            player1_carY = 570
        if player1_carX >=(roadEnd - 50):
            player1_carX= (roadEnd-50)
        if player1_carX <= roadStart:
            player1_carX = roadStart
        if player2_carY <= 500:
            player2_carY = 500
        if player2_carY >= 570:
            player2_carY = 570
        if player2_carX >=(roadEnd - 50):
            player2_carX= (roadEnd-50)
        if player2_carX <= roadStart:
            player2_carX = roadStart
        ...
        player1_list.update(player1_carX, player1_carY)
        player1_list.draw(gameDisplay)
        player2_list.update(player2_carX, player2_carY)
        player2_list.draw(gameDisplay)
        ...
        block_hit_list1 = pygame.sprite.spritecollide(player1, block_list, False)
        block_hit_list2 = pygame.sprite.spritecollide(player2, block_list, False)
        block_hit_list3 = pygame.sprite.spritecollide(player1, player2_list, False)


        for block in block_hit_list1:
            collide(player1_carX,player1_carY)
        for block in block_hit_list2:
            collide(player2_carX,player2_carY)
